...
async def get_history_by_id(session_id: str, user_id: str, _=Depends(verify_pin)):
...
async def delete_history(session_id: str, user_id: str, _=Depends(verify_pin)):
    if not await check_conversation_ownership(session_id, user_id):
...
async def rename_history(session_id: str, user_id: str, request: Request, _=Depends(verify_pin)):
    if not await check_conversation_ownership(session_id, user_id):
